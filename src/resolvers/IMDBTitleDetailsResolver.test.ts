...
const titlesToTest: ITitleTestData[] = [
    {
        ...
        casts: {
            ...
            tests: [
                ...
                {
                    index: null,
                    finderF: (item: IPersonDetails) => item.name.toLowerCase() === "kevin dorman",
                    name: "kevin dorman",
                    sourceId: null,
                    rolesLength: 2,
        ...
        test(
            ...
            async () => {
                ...
                if (testData.otherNamesMinLength !== undefined) {
                    expect(result.otherNames.length).toBeGreaterThan(testData.otherNamesMinLength);
                }
                ...
                if (testData.directors !== undefined) {
                    ...
                    expect(director.name.toLowerCase()).toBe(testData.directors.firstOneName);
                ...
                if (testData.mainRate !== undefined) {
                    ...
                    expect(mainRate.rate).toBeGreaterThanOrEqual(testData.mainRate.rate - 0.2);
                    expect(mainRate.rate).toBeLessThanOrEqual(testData.mainRate.rate + 0.2);
                    expect(mainRate.votesCount).toBeGreaterThan(testData.mainRate.minVotesCount);
                ...
                if (testData.dates !== undefined) {
                    ...
                    expect(result.dates.startDate.getFullYear()).toBe(testData.dates.startYear);
                ...
                if (testData.ageCategory !== undefined) {
                    expect(result.ageCategoryTitle.toLowerCase()).toBe(testData.ageCategory);
                }
                if (testData.languages !== undefined) {
                    Array.from(new Array(testData.languages?.length)).forEach((_v, i) => {
                ...
                if (testData.runtimeSeconds !== undefined) {
                    expect(result.runtime.seconds).toBeGreaterThanOrEqual(testData.runtimeSeconds);
                }
                ...
                if (testData.onOfKeywords !== undefined) {
                    expect(
                        result.keywords.map((i) => i.toLowerCase()).includes(testData.onOfKeywords)
                    ).toBe(true);
                }
                ...
                if (testData.postersMinLength !== undefined) {
                    expect(
                        result.allImages.filter((i) => i.type === "poster" && !!i.url).length
                    ).toBeGreaterThanOrEqual(testData.postersMinLength);
                }

                if (testData.stillFrameMinLength !== undefined) {
                    expect(
                        result.allImages.filter((i) => i.type === "still_frame" && !!i.url).length
                ...
                if (testData.quotesLength !== undefined) {
                    expect(result.quotes.length).toBeGreaterThanOrEqual(testData.quotesLength);
                }
                if (testData.spoilerQuotes !== undefined) {
                    expect(result.quotes.filter((i) => i.isSpoiler).length).toBeGreaterThanOrEqual(
